...
const Index = ({ auth, schools }) => {
    ...
    const trash = (s) => {
        if (confirm("Eliminar la escuela " + s.name + "? De veras?")) {
            fetch(route('school.destroy', { id: s.id }))
    ...
    function actions(data) {
        return (
            <div className="flex">
                ...
                <Link as="button" className="mx-1" href={ route('school.users', data.id) }><FaUserGroup /></Link>
    ...
    let topMenu = (
        <TopMenu auth={auth} title={ title } show={['school-add']} schoolAdd={ () => edit({})} />
    )
        ...
        <AuthenticatedLayout auth={auth} user={auth.user} header={ false } topMenu={ topMenu }>
            ...
            <Modal show={showForm} onClose={closeModal} >
                <div className={`bg-white p-4 shadow sm:rounded-lg`}>
                    <SchoolForm school={ selectedSource } afterSubmit={(n) => {console.log('hi', n); setShowForm(false)}} />
                </div>
            </Modal>
        </AuthenticatedLayout>
    )
