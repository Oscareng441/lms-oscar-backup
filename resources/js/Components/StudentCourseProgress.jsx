export default function StudentCourseProgress(props) {
    ...
    if (props.progress === null) {
        ...
        let bgCol =
            props.progress.pct_done > 99
                ? "bg-green-400"
                : props.progress.pct_done < 1
                  ? "bg-slate-200"
                  : "bg-green-200";
        txt = (
            <span
                className={`p-1 m-1 border rounded-md ${bgCol} text-sm`}
            >{`${props.progress.pct_done}% hecho; ${props.progress.score}% correctos; ${props.progress.total} problemas`}</span>
        );
        // let goBtn = ''
